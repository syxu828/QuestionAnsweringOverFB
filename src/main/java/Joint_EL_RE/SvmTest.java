...
public class SvmTest {
	...
	public static SvmTest instance = new SvmTest();
	
	public List<Triple> findBestTriple(List<Triple> triples){
		...
		try {  
            ...
            if( exitVal == 0 ){
            	...
            	for(int i = 0; i < items.size(); i++){
            		rankedTriples.add( triples.get(items.get(i).index) );
            		triples.get(items.get(i).index).sumScore = items.get(i).score;
            	}
