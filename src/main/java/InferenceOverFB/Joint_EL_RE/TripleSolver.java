    ...
    public List<List<Triple>> solve(String question){
        ...
        for(String subQuestion:subQuestions){
            ...
            for(String mention:mentions) {
                ...
                if (!question_EL.containsKey(question))
                    logger.info("there may be an error since the question does not have the corresponding entity linking resutls");

                HashMap<String, List<String>> el_results = question_EL.get(question);

                if (!el_results.containsKey(mention))
                    logger.info("there may be an error since the question does not have the corresponding mention");
